...
public:
	Link(int id) {
		...
		cost_ice_energy_ = 0;
		cost_bev_energy_ = 0;
		cost_phev_energy1_ = 0;
		cost_phev_energy2_ = 0;
		cost_hfcv_energy_ = 0;
		base_cost_travel_time_ = 0;
		base_cost_ice_ = 0;
		base_cost_bev_ = 0;
		base_cost_phev1_ = 0;
		base_cost_phev2_ = 0;
		...
		predicted_cost_travel_time_.resize(number_of_predictions_, 0);
		predicted_cost_ice_.resize(number_of_predictions_, 0);
		predicted_cost_bev_.resize(number_of_predictions_, 0);
		predicted_cost_phev1_.resize(number_of_predictions_, 0);
		predicted_cost_phev2_.resize(number_of_predictions_, 0);
		predicted_cost_hfcv_.resize(number_of_predictions_, 0);
	}

	int id_;
	double cost_travel_time_;
	double cost_ice_energy_;
	double cost_bev_energy_;
	double cost_phev_energy1_;
	double cost_phev_energy2_;
	double cost_hfcv_energy_;
	double base_cost_travel_time_;
	double base_cost_ice_;
	double base_cost_bev_;
	double base_cost_phev1_;
	double base_cost_phev2_;
	double base_cost_hfcv_;
	vector<double> predicted_cost_travel_time_;
	vector<double> predicted_cost_ice_;
	vector<double> predicted_cost_bev_;
	vector<double> predicted_cost_phev1_;
	vector<double> predicted_cost_phev2_;
	vector<double> predicted_cost_hfcv_;
...
public:
	Network() {
		...
		price_electric_ = 0.12; // define electric price here
	}
	static unordered_map<int, Vehicle> map_vehicles_;
	static unordered_map<int, Link> map_links_;
	static double overall_travel_time_;
	static double overall_fuel_consumed_;
	static unordered_map<VehicleType, double> travel_time_per_vehicle_type_;
	static unordered_map<VehicleType, double> fuel_consumed_per_vehicle_type_;
	static int N_links_;
	static int N_vehicles_;
	static int N_vehicle_types_;	// only count self-defined vehicle type
	static double price_gas_;		// US dollar per gallon
	static double price_electric_;	// US dollar per kWh
};
...
int AAPIInit()
{
	...
	for (int i = 0; i < network.N_links_; i++) {
		...
		Emission(spd, grade, 0.0, network.map_links_[secid].base_cost_ice_, network.map_links_[secid].base_cost_bev_, network.map_links_[secid].base_cost_phev1_,
			network.map_links_[secid].base_cost_phev2_, network.map_links_[secid].base_cost_hfcv_);

		double _free_flow_travel_time = secinf.length / spd;
		network.map_links_[secid].base_cost_travel_time_ = _free_flow_travel_time;

		network.map_links_[secid].base_cost_ice_ *= _free_flow_travel_time;
		network.map_links_[secid].base_cost_bev_ *= _free_flow_travel_time;
		network.map_links_[secid].base_cost_phev1_ *= _free_flow_travel_time;
		network.map_links_[secid].base_cost_phev2_ *= _free_flow_travel_time;
		network.map_links_[secid].base_cost_hfcv_ *= _free_flow_travel_time;


		ANGConnSetAttributeValueInt(link_attribute_ice, secid, ice_base_energy_per_link[secid]);
		ANGConnSetAttributeValueInt(link_attribute_bev, secid, bev_base_energy_per_link[secid]);
		ANGConnSetAttributeValueInt(link__attribute_phev, secid, phev_base1_energy_per_link[secid]);
		ANGConnSetAttributeValueInt(link_attribute_hfcv, secid, hfcv_base_energy_per_link[secid]);
		ANGConnSetAttributeValueInt(link_attribute_travel_time, secid, secinf.length / spd);
	}
